...
subroutine _qrm_min_norm2d(qrm_mat, b, x)
  ...
  if(qrm_mat%n .lt. qrm_mat%m) then
     call qrm_err_push(31, name,ied=(/qrm_mat%m,qrm_mat%n,0,0,0/))
     goto 9999
  end if

  ! analysis
  call _qrm_analyse(qrm_mat, 't')
  __QRM_CHECK_RET(name,'qrm_analyse',9999)
  ! factorization
  call _qrm_factorize(qrm_mat, 't')
  __QRM_CHECK_RET(name,'qrm_factorize',9999)

  call _qrm_solve2d(qrm_mat, 't', b, x)
  __QRM_CHECK_RET(name,'qrm_solve',9999)
  call _qrm_apply2d(qrm_mat, 'n', x)
...
subroutine _qrm_min_norm1d(qrm_mat, b, x)
  ...
  call _qrm_solve1d(qrm_mat, 't', b, x)
  __QRM_CHECK_RET(name,'qrm_solve',9999)
  call _qrm_apply1d(qrm_mat, 'n', x)
  __QRM_CHECK_RET(name,'qrm_apply',9999)
