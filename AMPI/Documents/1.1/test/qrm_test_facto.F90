...
subroutine _qrm_test_facto(c, m)
  use _qrm_coverage_mod, protect => _qrm_test_facto
  implicit none
...
contains
  ...
  subroutine case1(m)
    ...
    do i=11, size(matrices)
       ...
       do j=1, 4
          ...
          call qrm_set(qrm_spmat,'qrm_nthreads',procs(j))
          ...
          ! print message
          call _qrm_prnt_testmesg(5, 1, j, i, (info .eq. 0) .and. (err .lt. eps))
  ...
  !> @brief Test the singletons featurefactorization with different
  !! numbers of ib/nb
  subroutine case2(m)
    use _qrm_mod
    implicit none
    ...
    do i=11, size(matrices)
       ...
       do j=1, 4
          ...
          call qrm_set(qrm_spmat,'qrm_ib',ib(j))
          call qrm_set(qrm_spmat,'qrm_nb',nb(j))
          ...
          ! print message
          call _qrm_prnt_testmesg(5, 2, j, i, (info .eq. 0) .and. (err .lt. eps))
