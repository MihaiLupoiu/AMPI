...
void TTFT_init(){
  ...
  for (unsigned int l = 0; l < ps_args->ui_q ; l++ ){
    ...
    for (unsigned int i = l+1; i < ps_args->ui_p ; i++ ){
      ...
      facto = new_task(F, ps_args,NULL,i, l,0,0);

      
      elim = new_task(Z,ps_args,NULL, i, l, l,0);
      
      for (unsigned int j = l+1; j < ps_args->ui_q ; j++ ){
        
        update = new_task(H,ps_args,facto, i,l, j,0);
        
        update = new_task(V,ps_args,elim, i,l, l,j); 
      }
